...
function SalesPage() {
  ...
  const handleDelete = (r: Invoice) => {
    ...
    // Reverse whatever referral commission this sale granted, on both the
    // party and their referrer — mirrors the stock-restore above.
    if (live.referralCommission) {
      const referrerId = PartyRepo.get(live.partyId)?.referredBy;
      PartyRepo.adjustFieldBatched(batch, live.partyId, "referralWalletBalance", -live.referralCommission);
      if (referrerId) {
